...
public class _004_ConsumerApp {
    ...
    public static void main(String[] args) {
        ...
        String topicName = "my_topic";// args[0].toString();
    ...
    private static void printSet(Set<TopicPartition> collection) {
        if (collection.isEmpty()) {
            ...
            for (TopicPartition partition : collection) {
                System.err.println(String.format("Partition: %s in Topic: %s", Integer.toString(partition.partition()), partition.topic()));
            }
        }
    }
    ...
    private static void printRecords(ConsumerRecords<String, String> records) {
        for (ConsumerRecord<String, String> record : records) {
            System.err.println(String.format("Topic: %s, Partition: %d, Offset: %d, Key: %s, Value: %s", record.topic(), record.partition(), record.offset(), record.key(), record.value()));
        }
    }
}